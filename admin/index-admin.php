<?php
// 1. Nhúng tệp cấu hình kết nối cơ sở dữ liệu qua PDO
...
try {
    ...
    $stmt_rev = $conn->query("SELECT SUM(total_price) FROM orders WHERE status = 'Đã giao'");
    $total_revenue = $stmt_rev->fetchColumn() ?? 0;

   
    $stmt_ord = $conn->query("SELECT COUNT(id) FROM orders WHERE status = 'Chờ xác nhận'");
    $pending_orders = $stmt_ord->fetchColumn() ?? 0;

    
    $stmt_prod = $conn->query("SELECT COUNT(id) FROM products");
    $total_products = $stmt_prod->fetchColumn() ?? 0;

    $stmt_stock = $conn->query("SELECT pv.*, p.product_name 
    ...
    // Trường hợp gặp sự cố truy vấn, hiển thị hộp thoại cảnh báo lỗi hệ thống
    echo '<div class="alert alert-danger">Lỗi kết xuất dữ liệu báo cáo: ' . $e->getMessage() . '</div>';
}
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="page-title m-0">Bảng Điều Khiển Tổng Quan</h2>
    <span class="text-secondary fw-semibold font-monospace bg-white px-3 py-1 border rounded shadow-sm">
...
<div class="row g-4 mb-5">
    ...
    <div class="col-12 col-md-6 col-xl-4">
        <div class="card stat-card bg-white p-4 border-start border-primary border-4 rounded-3">
    ...
    <div class="col-12 col-md-6 col-xl-4">
        <div class="card stat-card bg-white p-4 border-start border-warning border-4 rounded-3">
            <div class="d-flex justify-content-between align-items-center">
                <div>
            ...
            <a href="modules/don-hang/index.php" class="small text-warning mt-2 text-decoration-none fw-semibold">
    ...
    <div class="col-12 col-md-6 col-xl-4">
        <div class="card stat-card bg-white p-4 border-start border-success border-4 rounded-3">
            <div class="d-flex justify-content-between align-items-center">
                <div>
            ...
            <a href="modules/san-pham/index.php" class="small text-success mt-2 text-decoration-none fw-semibold">
                    ...
                    <tbody>
                        <?php foreach($low_stock_items as $item): ?>
                            <tr>
                                ...
                                <td class="fw-bold fs-6"><?= $item['size'] ?></td>
                                <td class="text-danger fw-bold fs-6"><?= $item['stock'] ?> đôi</td>
                                <td>
                                    <?php if($item['stock'] == 0): ?>
                                        <span class="badge bg-danger bg-opacity-10 text-danger badge-soft px-3 py-2 fw-bold">
                                            CHÁY HÀNG HOÀN TOÀN
                                        </span>
                                    <?php else: ?>
                                        <span class="badge bg-warning bg-opacity-10 text-warning badge-soft px-3 py-2 fw-bold">
                                            SẮP CHÁY HÀNG
                                        </span>
                                    <?php endif; ?>
                                </td>
                                <td class="pe-4">
                                    <a href="modules/kho-hang/nhap-hang.php?variant_id=<?= $item['id'] ?>" 
                                       class="btn btn-outline-dark btn-sm fw-bold px-3 py-1.5 transition-all">
...
<?php
// 3. Nhúng bộ khung giao diện Footer kết thúc trang quản trị
